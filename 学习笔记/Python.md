...
2. 如何使用 python 解释器
   1. 唤出解释器：
    ...
    2. 交互模式
## 有关 python 环境
1. python 环境是指python 程序运行所需的一系列功能库，这些库依赖于一个核心的 python 解释器而存在，与解释器捆绑在一起。可以说，python 程序的执行成功与否，一半取决于代码内部逻辑，另一半则取决于 python 环境的配置是否合适。
...
4. 库(Library)：由模块和包构成，在所针对的功能上更加具有完整性，可以分为**标准库**和**第三方库**
   1. 标准库：是 python 内置的一些模块和包，分为**内置方法**、**内置模块**和**纯 Python 模块和包**
