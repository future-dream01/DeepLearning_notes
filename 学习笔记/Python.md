...
2. 语句
   ...
   - 复合语句：
      ...
      - **match**
3. 自定义类
   - 地位：类，体现了编程领域的重要思想：**面向对象编程**，python 中万物皆为对象，遵循这一思想，类是定义了某种**实例对象**可以具有的**属性**和可以使用的**方法**的"类对象"，我们于是将问题的解决转而以对象为中心，将任务拆分并分配给了功能不同、各司其职的对象，各自解决，随着我们对类对象中属性和方法的补充，每个实例对象功能越来越完整，可以完成的任务越来越专一、复杂，问题也就好解决了
   - 结构：类中只有两类关系：**类型-实例关系**、**父类-子类关系**
     - ```python
       class Animal():
            number=0                               # 类变量
      ...
      ```
     - **类名**: 一般是开头第一个字母大写的标识符
     - **类属性**:类对象自己的属性，与类本身以及其下所有实例对象共享，用于记录与类本身相关的特征。一般直接写在类定义中的`class 类名():`之下，推荐使用类对象名调用，也可以使用实例对象名调用
     - **类方法**: 类对象自己的方法，用来修改类属性，使用`@classmethod`装饰器定义，指定参数是`cls`,表示类本身，推荐使用类对象名调用，也可以使用实例对象名调用
   ...
   - 特点(面相对象编程)：
     - 封装:是将属性(数据)和方法(操作数据的函数)绑定在一起，并隐藏内部实现细节的方法。封装分为两层，第一层，即类的形式本身，将属性和方法绑定到某类对象上，以提高代码编写效率与复用性；第二层，即类内将一些属性和方法私有化，隐藏起来，仅对外提供一些公共接口供执行某些有限的操作，避免了对内部数据不必要的影响，提高代码可维护性
...
5. 常用的第三方库：
   - **Matplotlib** ：常用的绘图库
     - pyplot模块：
       ...
       - `scatter(x,y,label,color=None,marker='o')`:绘制散点图，**x**为存放x轴数据的列表，**y**为存放y轴数据的列表，(这里注意和plot区分：此处必须指定x和y两者，否则无法绘制)**label**是对这根曲线的命名,**color**为线的颜色，可指定'r'、'g'、'b'，**marker**指定数据点的形状，'o'为圆点，'s'为方块，,'*'为星形,'+'为加号,'v'为下三角
